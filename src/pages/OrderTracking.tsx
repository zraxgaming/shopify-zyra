...
const OrderTracking = () => {
  ...
  const canAutoRefund = (created_at: string) => {
    if (!created_at) return false;
    const created = dayjs(created_at);
    const now = dayjs();
    return now.diff(created, 'hour', true) <= 3;
  };
  ...
  const handleAutoRefund = async () => {
    ...
    setRefundError('');
    try {
      const token = process.env.NEXT_PUBLIC_ZIINA_TOKEN || '';
      if (!token) throw new Error('Ziina API token not set');
      if (!order.payment_intent_id || !order.currency_code) throw new Error('Missing payment info for refund');
      const refundData = await requestZiinaRefund({
        id: order.id,
      ...
      const { error } = await supabase
        .from('orders')
        .update({ status: 'canceled', refund_reason: 'Auto-refunded via Ziina', refund_transaction_id: refundData.id })
        .eq('id', order.id);
      if (error) throw error;
      setRefundSuccess(true);
      toast({ title: 'Order refunded and canceled via Ziina.' });
    } catch (err: any) {
  ...
  const handleManualRefund = async () => {
    if (!order || !refundMessage.trim()) return;
    setRefundLoading(true);
    setRefundError('');
    try {
      const { error } = await supabase
        .from('orders')
        .update({ refund_reason: refundMessage, refund_requested: true })
        .eq('id', order.id);
      if (error) throw error;
      setRefundSuccess(true);
      toast({ title: 'Refund request sent for review.' });
    } catch (err: any) {
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content="https://yourdomain.com/order-tracking" />
        <meta property="og:image" content="https://yourdomain.com/og-image.png" />
        <link rel="canonical" href="https://yourdomain.com/order-tracking" />
      ...
      <Container className="py-12">
        <div className="max-w-2xl mx-auto">
          ...
          {/* ...existing search and order display code... */}
          {order && order.status !== 'canceled' && (
            <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 mt-6">
              <CardHeader>
                <CardTitle className="text-gray-900 dark:text-gray-100">Refund or Cancel Order</CardTitle>
              </CardHeader>
              <CardContent>
                {canAutoRefund(order.created_at) ? (
                  <div className="space-y-2">
                    <p className="text-gray-600 dark:text-gray-400">This order is eligible for automatic refund within 3 hours of creation.</p>
                    <Button onClick={handleAutoRefund} disabled={refundLoading || refundSuccess} className="bg-red-600 hover:bg-red-700 text-white">
                      {refundLoading ? 'Processing...' : refundSuccess ? 'Refunded' : 'Refund & Cancel Order'}
                    </Button>
                    {refundError && <p className="text-red-500 text-sm">{refundError}</p>}
                  </div>
                ) : (
                  <div className="space-y-2">
                    <p className="text-gray-600 dark:text-gray-400">To request a refund or cancellation, please provide your reason below. Our team will review your request.</p>
                    <textarea
                      className="w-full border rounded p-2 text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800"
                      rows={3}
                      placeholder="Enter your reason for refund/cancellation..."
                      value={refundMessage}
                      onChange={e => setRefundMessage(e.target.value)}
                      disabled={refundSuccess}
                    />
                    <Button onClick={handleManualRefund} disabled={refundLoading || refundSuccess || !refundMessage.trim()} className="bg-yellow-600 hover:bg-yellow-700 text-white">
                      {refundLoading ? 'Sending...' : refundSuccess ? 'Request Sent' : 'Send Refund Request'}
                    </Button>
                    {refundError && <p className="text-red-500 text-sm">{refundError}</p>}
                  </div>
                )}
